  ...
  <div class="panel panel-default">
    <div class="panel-heading">กรอกข้อมูลการจองห้องประชุม</div>
      <div class="panel-body">
        <form class="form-horizontal" action="{{ url('control/reserve_adayinweek/confirm') }}" id="form_adayinweek" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10" id="date_list">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input class="btn btn-success" id="submit" type="submit" value="บันทึก">
            </div>
          </div>
        </form>
      </div>
...
<script>
  ...
  var data_reserve = <?php echo json_encode($date_reserve) ?>;
  var time_reserve = <?php echo json_encode($time_reserve) ?>;
...
function setDateReserveList(data_reserve){
  ...
  for(index in data_reserve){
    content += '<p>'+dateDBtoBC(data_reserve[index])+'  เวลา '+time_reserve[index][0].substring(0,5)+' - '+time_reserve[index][1].substring(0,5)+'</p>';
  }
  $('#date_list').html(content)
  $('#data_reserve').val(JSON.stringify(data_reserve))
  $('#time_reserve').val(JSON.stringify(time_reserve))
}

function dateDBtoBC(date){
  var res = date.split("-")
